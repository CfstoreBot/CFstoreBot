...
def dynamic_buttons(update: Update, context: CallbackContext):
    ...
    if retorno[0] == '^' or retorno in lista:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f'🔙︎ {button_back_edit_menu}', callback_data='cancelar_edicao1')], ])
        ...
        if retorno[0] == '^':
            if retorno[0] == '^':
                ...
                context.bot.edit_message_text(chat_id=query.message.chat_id, message_id=query.message.message_id,
                                              text=price_edit_info_1.format(category, categoria, new_price,
                                                                            price) + '\n\n' + '-=' * 20 + f'\n\n{price_edit_info_2}: {price_edit_info_null}',
                                              parse_mode='Markdown', reply_markup=numpad3)
            ...
            categoria1 = text.strip().replace('`', '')[text.find(category) + len(category) + 2:].strip()
            categoria2 = categoria1[:categoria1.find('\n')].replace(f'({new})', '').strip()
            q = text.replace(price_edit_info_null, '').strip().replace('`', '')[
                text.find(price_edit_info_2.replace('*', '')) + len(price_edit_info_2):]
            ...
            context.bot.edit_message_text(chat_id=query.message.chat_id, message_id=query.message.message_id,
                                          text=price_edit_info_changed.format(category, categoria2, preco_antigo,
                                                                              preco_novo), parse_mode='Markdown',
            ...
            q = text.replace(price_edit_info_null, '').strip().replace('`', '')[
                text.find(price_edit_info_2.replace('*', '')) + len(price_edit_info_2):]
            context.bot.edit_message_text(chat_id=query.message.chat_id, message_id=query.message.message_id,
                                          text=price_edit_info_1.format(category, categoria2, new_price,
                                                                        price) + '\n\n' + '-=' * 20 + f'\n\n{price_edit_info_2}: {price_edit_info_null}',
                                          parse_mode='Markdown', reply_markup=numpad3)
                ...
                if int(q_add) < 201:
                    if int(q_add) < 21:
                        context.bot.edit_message_text(chat_id=query.message.chat_id,
                                                      ...
                                                      text=price_edit_info_1.format(category, categoria2, new_price,
                                                                                    price) + f'\n\n{price_edit_info_2}: {q_add}',
                                                      parse_mode='Markdown', reply_markup=numpad)
                        ...
                        context.bot.edit_message_text(chat_id=query.message.chat_id,
                                                      ...
                                                      text=price_edit_info_1.format(category, categoria2, new_price,
                                                                                    price) + f'\n\n{price_edit_info_2}: {q_add}',
                                                      parse_mode='Markdown', reply_markup=numpad2)
        ...
        keyboard2 = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f'🔙︎ {button_back_edit_menu}', callback_data='cancelar_edicao2')], ])
        ...
        if retorno[0] == '+':
            if retorno[0] == '+':
                ...
                context.bot.edit_message_text(chat_id=query.message.chat_id, message_id=query.message.message_id,
                                              text=price_edit_info_1.format(amount, quantidade, new_price,
                                                                            price) + '\n\n' + '-=' * 20 + f'\n\n{price_edit_info_2}: {price_edit_info_null}',
                                              parse_mode='Markdown', reply_markup=numpad3)
                ...
                price = asyncio.run(pesquisar_mix(quantidade.replace('+', '')))[1]
                context.bot.edit_message_text(chat_id=query.message.chat_id, message_id=query.message.message_id,
                                              text=price_edit_info_1.format(amount, quantidade2, new_price,
                                                                            price) + '\n\n' + '-=' * 20 + f'\n\n{price_edit_info_2}: {price_edit_info_null}',
                                              parse_mode='Markdown', reply_markup=numpad3)
            ...
            context.bot.edit_message_text(chat_id=query.message.chat_id, message_id=query.message.message_id,
                                          text=price_edit_info_changed.format(amount, quantidade2, preco_antigo,
                                                                              preco_novo), parse_mode='Markdown',
            ...
            context.bot.edit_message_text(chat_id=query.message.chat_id, message_id=query.message.message_id,
                                          text=price_mix_deleted.format(amount, quantidade2), parse_mode='Markdown',
                                          reply_markup=keyboard2)
    ...
    if check_status(user_id):
        if retorno[0] == '~':
            ...
            if contador == 1:
                ...
                keyboard.append([InlineKeyboardButton(f'🔙 {button_back}', callback_data='unitaria')])

                context.bot.edit_message_text(chat_id=query.message.chat_id, message_id=query.message.message_id,
                                              text=cc_info_buy.format(numero, expiracao, bandeira, tipo, categoria,
                                                                      banco, preco_cc), parse_mode='Markdown',
                                              reply_markup=InlineKeyboardMarkup(keyboard))
            ...
            if row is not None:
                ...
                if not int(preco_cc) <= int(saldo):
                    query.bot.answer_callback_query(update.callback_query.id,
                                                    text=no_credits_alert.format(preco_cc, saldo), show_alert=True)
                    ...
                    if asyncio.run(check_comprada(numero)):
                        ...
                        if asyncio.run(check_config('checker'))[1] == '1' and asyncio.run(check_config('auto_live'))[
                            1] == '0':
                            check = checker(credit_card)
                            if check[0]:
                                cc_usada(credit_card)

                                keyboard2 = [[InlineKeyboardButton(f'🔄 Solicitar troca',
                                                                   callback_data=f'troca|{t}|{categoria}|{cc_id}'), ]]

                                status = f'\n\n✅ *Status*: `{check[1]}`'

                                update_cc = asyncio.run(update_cartao(cc_id, user_id, hora))
                                context.bot.edit_message_text(chat_id=query.message.chat_id,
                                                              message_id=query.message.message_id, text=cc_buy_warning,
                                                              parse_mode='Markdown',
                                                              reply_markup=InlineKeyboardMarkup(keyboard))
                                if asyncio.run(check_config('troca'))[1] == '1':
                                    a = query.bot.send_message(chat_id=query.message.chat.id,
                                                               text=f"✨ | *DETALHES DO CARTÃO*:\n\n💳 *Cartão*: `{numero}|{expiracao.replace('/', '|')}|{cvv}`\n🏳 *Bandeira*: `{bandeira}`\n⭐ *Nivel*: `{categoria}`\n🏦 *Banco*:  `{banco}`\n\n👤 *Nome*: `{nome}`\n📃 *CPF*: `{cpf}`" + status,
                                                               parse_mode='Markdown',
                                ...
                                for grupo in grupos:
                                    try:
                                        query.bot.send_message(chat_id=grupo,
                                                               text=f'💳 | *Cartão comprado!*\n\n⭐ *Nivel*: `{categoria}`\n👤 *Comprador*: `{usernome}`',
                                                               parse_mode='Markdown')
                                    except:
                                        pass
                                ...
                                context.bot.edit_message_text(chat_id=query.message.chat_id,
                                                              message_id=query.message.message_id,
                                                              text="💳 | *Cartão recusado!*\n\nCartão recusado! Escolha um outro cartão para comprar!",
                                                              parse_mode='Markdown',
                                                              reply_markup=InlineKeyboardMarkup(keyboard))
                        ...
                        if asyncio.run(check_config('checker'))[1] == '1' and asyncio.run(check_config('auto_live'))[
                            ...
                            context.bot.edit_message_text(chat_id=query.message.chat_id,
                                                          message_id=query.message.message_id,
                                                          text='Estamos efetuando a verificação desse cartão, por favor aguarde!',
                                                          parse_mode='Markdown')
                            check = checker(credit_card)
                            if check[0]:
                                cc_usada(credit_card)
                                ...
                                if int(preco_cc) <= int(saldo):
                                    ...
                                    context.bot.edit_message_text(chat_id=query.message.chat_id,
                                                                  message_id=query.message.message_id,
                                                                  text=cc_buy_warning, parse_mode='Markdown',
                                                                  reply_markup=InlineKeyboardMarkup(keyboard))
                                    if asyncio.run(check_config('troca'))[1] == '1':
                                        a = query.bot.send_message(chat_id=query.message.chat.id,
                                                                   text=f"✨ | *DETALHES DO CARTÃO*:\n\n💳 *Cartão*: `{numero}|{expiracao.replace('/', '|')}|{cvv}`\n🏳 *Bandeira*: `{bandeira}`\n⭐*Nivel*: `{categoria}`\n🏦 *Banco*:  `{banco}`\n\n👤 *Nome*: `{nome}`\n📃 *CPF*: `{cpf}`" + status,
                                    ...
                                    query.bot.send_message(chat_id=user_id,
                                                           text=f'💳 | *O cartão não pode ser comprado por saldo insuficiente!*\n\nO cartão não foi comprado por você já não ter mais o saldo na carteira para isso, provavelmente porque o saldo da sua carteira foi usado para compra de outra CC nesse meio tempo. Contate o suporte caso esse seja um mal entendido!',
                                                           parse_mode='Markdown')
                                ...
                                context.bot.edit_message_text(chat_id=query.message.chat_id,
                                                              message_id=query.message.message_id,
                                                              text='💳 |  *Escolhendo outro cartão...*\n\nPedimos que aguarde, pois esse cartão foi recusado, mas estamos escolhendo um outro cartão equivalente para você, por favor aguarde! Isso pode levar um tempo...',
                                                              parse_mode='Markdown')
                                while True:
                                    print(tipo_p)
                                    if tipo_p.strip() == '':
                                    ...
                                    try:
                                        if not contador_die >= 20:
                                            ...
                                            if cc[0]:
                                                ...
                                                if tipo_p.strip() == '':
                                                    context.bot.edit_message_text(chat_id=query.message.chat_id,
                                                                                  message_id=query.message.message_id,
                                                                                  text='💳 | *Sem cartões para a categoria escolhida!*\n\nO estoque da categoria do cartão comprado acabou, e não podemos pegar uma CC equivalente, tente escolher outra CC ou chame o suporte caso o problema percista!',
                                                                                  parse_mode='Markdown',
                                                                                  reply_markup=InlineKeyboardMarkup(
                                                                                      keyboard))
                                                    ...
                                                    context.bot.edit_message_text(chat_id=query.message.chat_id,
                                                                                  message_id=query.message.message_id,
                                                                                  text=f'💳 | *Cartões com {tipo_p} especifico(a) acabaram*\n\nO estoque com o(a) {tipo_p} especifico(a) acabou, pesquise por outro cartão ou relecione outro cartão!',
                                                                                  parse_mode='Markdown',
                                                                                  reply_markup=InlineKeyboardMarkup(
                                                                                      keyboard))
                                            ...
                                            context.bot.edit_message_text(chat_id=query.message.chat_id,
                                                                          message_id=query.message.message_id,
                                                                          text='💳 | *Limite ultrrapassado!*\n\nDemorou muito tempo desde a sua compra, por favor, escolha outro level para continuar a comprar CCs ou chame o suporte caso o problema percista!',
                                                                          parse_mode='Markdown',
                                                                          reply_markup=InlineKeyboardMarkup(keyboard))
                                            break
                                        ...
                                        context.bot.edit_message_text(chat_id=query.message.chat_id,
                                                                      message_id=query.message.message_id,
                                                                      text='💳 | *Cartão não comprado!*\n\nTente escolher outra CC ou chame o suporte caso o problema percista!',
                                                                      parse_mode='Markdown',
                                                                      reply_markup=InlineKeyboardMarkup(keyboard))
                                        break


                        elif asyncio.run(check_config('checker'))[1] == '0' and asyncio.run(check_config('auto_live'))[
                            1] == '0':
                            update_cc = asyncio.run(update_cartao(cc_id, user_id, hora))
                            ...
                            if update_cc:
                                ...
                                context.bot.send_message(chat_id=query.message.chat_id,
                                                         text=cc_info.format(numero, expiracao, cvv, bandeira, tipo,
                                                                             categoria, banco, cpf, nome, preco_cc),
                                                         parse_mode='Markdown')
                                asyncio.run(subtrair_saldo(user_id, preco_cc))
                                ...
                                context.bot.edit_message_text(chat_id=query.message.chat_id,
                                                              message_id=query.message.message_id,
                                                              text=f'❌ | {cc_buy_error_2}', parse_mode='Markdown',
                                                              reply_markup=InlineKeyboardMarkup(keyboard))


                    else:
                        query.bot.answer_callback_query(update.callback_query.id, text=f'❕ {cc_buy_error_1}',
                                                        show_alert=True)
        ...
        try:
            context.bot.edit_message_text(chat_id=query.message.chat_id, message_id=query.message.message_id,
                                          text=denied_text(user_id), parse_mode='Markdown')
        except:
