...
def troca(update: Update, context: CallbackContext):
    ...
    if t[0] == True and int(t[1].split(':')[0]) <= tempo_de_troca:
        if asyncio.run(check_config('checker'))[1] == '1':
            a = query.bot.send_message(chat_id=query.message.chat.id, text=f"Estamos verificando a possibilidade de troca...", parse_mode='Markdown')
            recheck = checker(cc)
            if recheck[0] == False:
                context.bot.edit_message_text(chat_id=query.message.chat_id, message_id=a.message_id, text=f"A sua CC está sendo trocada, por favor, aguarde...", parse_mode='Markdown')
                while True:
                    contador_die = 0
                    contador_die += 1
                    repetidos = []
                            ...
                            if cc[0] == True and not cc[1] == []:
                                ...
                                if not credit_card in repetidos:
                                    ...
                                    if check[0] == True and not a_cc_foi_usada(credit_card):
                                        ...
                                        status = f'\n\n✅ *Status*: `CC LIVE, troca recusada.`'

                                        query.bot.send_message(chat_id=query.message.chat.id, text=f"💳 | *DADOS DO CARTÃO*:\n*NOTA*: Sua CC foi trocada com sucesso!\n\n✅ GARANTIMOS SOMENTE LIVE!\n⚠️ NÃO GARANTIMOS A APROVAÇÃO\n⚠️ NÃO GARANTIMOS SALDO\n\n💳 *Cartão*: `{numero}|{expiracao.replace('/', '|')}|{cvv}`\n🏳 *Bandeira*: `{bandeira}`\n⭐ *Nivel*: `{categoria}`\n🏦 *Banco*:  `{banco}`\n\n👤 *Nome*: `{nome}`\n📃 *CPF*: `{cpf}`"+status, parse_mode='Markdown')
