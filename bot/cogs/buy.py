...
def comprar(update: Update, context: CallbackContext):
    ...
    if check_status(user_id):
        ...
        if asyncio.run(check_config('afiliado'))[1] == '1':
            keyboard.append([InlineKeyboardButton(f'💳 {button_unitary_card}', callback_data='unitaria'.format(str(())))])
        ...
        keyboard.append([InlineKeyboardButton(f'🔙 {button_main}', callback_data='main')])
