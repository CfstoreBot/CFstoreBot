...
def start(update: Update, context: CallbackContext):
    ...
    if check_status(user_id):
        ...
        if asyncio.run(check_config('afiliado'))[1] == '1':
            keyboard.append([InlineKeyboardButton(f'💳 {button_buy}', callback_data='m1'.format(str(support().replace('@', ''))))])
            keyboard.append([InlineKeyboardButton(f'💵 {button_add_credits}', callback_data='adicionar_saldo'), InlineKeyboardButton(f'📃 {button_history}', callback_data='m3')])
            keyboard.append([InlineKeyboardButton(f'👤 {button_info}', callback_data='m2')])
...
def menu(update: Update, context: CallbackContext):
    ...
    if asyncio.run(check_config('afiliado'))[1] == '1':
        keyboard.append(
            [InlineKeyboardButton(f'💳 {button_buy}', callback_data='m1'.format(str(support().replace('@', ''))))])
    keyboard.append([InlineKeyboardButton(f'💵 {button_add_credits}', callback_data='adicionar_saldo'),
